 ...
 */
export default class SubhostingClient {
  ...
  constructor(accessToken?: string, orgId?: string, options?: ClientOptions) {
    ...
    this.offlineMode = Deno.env.get("MODE") === "offline";
  }
  ...
  async listProjects(query?: any): Promise<Response> {
    if (this.offlineMode) {
      return new Response(JSON.stringify([
        {
          id: "0530ee66-f5d7-429e-9333-26fcccb9b664",
          name: "wide-boa-38",
        },
        {
          id: "6cef7371-2d5a-4d2a-a2bd-35cf5aa358b2",
          name: "brave-starling-15",
        },
      ]));
    }
  ...
  async listDeployments(projectId: string, query?: any): Promise<Response> {
    if (this.offlineMode) {
      return new Response(JSON.stringify([
        { id: "fake-deployment", domains: ["http://0.0.0.0:8000/"] },
      ]));
    }
    const qs = new URLSearchParams(query).toString();
